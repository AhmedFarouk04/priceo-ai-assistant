...
function resolveProductSearchParams(message, params = {}) {
  const hasKeyword = Object.prototype.hasOwnProperty.call(params, "keyword");
  const hasSort = Object.prototype.hasOwnProperty.call(params, "sort");
  const hasLimit = Object.prototype.hasOwnProperty.call(params, "limit");
  const hasPriceGte = Object.prototype.hasOwnProperty.call(params, "priceGte");
  const hasPriceLte = Object.prototype.hasOwnProperty.call(params, "priceLte");
  const hasRatingGte = Object.prototype.hasOwnProperty.call(params, "ratingGte");

  return {
    keyword: hasKeyword ? params.keyword : extractKeyword(message),
    sort: hasSort ? params.sort : "",
    limit: hasLimit ? params.limit : 10,
    priceGte: hasPriceGte ? params.priceGte : null,
    priceLte: hasPriceLte ? params.priceLte : null,
    ratingGte: hasRatingGte ? params.ratingGte : null,
  };
}
...
export async function routeIntent({ intent, message, role, token, params = {} }) {
  const base = { message, role, hasToken: Boolean(token) };

  switch (intent) {
    case "product_search": {
      const searchParams = resolveProductSearchParams(message, params);
    ...
    case "product_details": {
      const id = params?.id || extractEntityId(message);
      if (!id) {
        return {
          source: "assistant",
          data: {
    ...
    case "create_complaint":
      {
        const subject = params?.subject || extractComplaintSubject(message);
        return {
          source: "assistant",
          data: {
            ...base,
